...
class DataCollatorWithPositionIDsMasking(DataCollator):
    ...
    def __call__(self, features: Sequence[Dict[str, "torch.Tensor"]]) -> Dict[str, "torch.Tensor"]:
        ...
        for input_name in features[0].keys():
            if input_name in ("input_ids", "attention_mask", "labels", "position_ids"):
                ...
                if input_name == "input_ids":
                    original_input_ids = [feature[input_name].clone() for feature in features]
                    ...
                    batch["_original_input_ids"] = torch.cat(original_input_ids, dim=-1).unsqueeze(0)
                else:
                    batch[input_name] = torch.cat([feature[input_name] for feature in features], dim=-1).unsqueeze(0)
            else:
                batch[input_name] = default_collate([feature[input_name] for feature in features])
        ...
        if "labels" in batch:
            ...
            # if "_original_input_ids" in batch:
            #     mask_mask = batch["input_ids"] == self.mask_token_id
            #     batch["labels"][mask_mask] = batch["_original_input_ids"][mask_mask]
            #     batch["labels"][~mask_mask] = IGNORE_INDEX
            #     del batch["_original_input_ids"]
            # else:
            #     batch["labels"][~mask_mask] = IGNORE_INDEX
        return batch
@dataclass
