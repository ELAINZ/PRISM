        ...
        if labels is not None:
            ...
            if is_liger_kernel_available():
                if mask_ratio is not None:
                    ...
                    loss_mask_1 = loss > 0
                    print(f"[DEBUG] loss mask 1: {loss_mask != loss_mask_1}")
                    exit()
                    # print(f"[DEBUG] loss mask: {loss_mask}")
                    # print(f"[DEBUG] loss mask sum: {(loss * loss_mask).sum()}")
                    # loss = (loss * loss_mask * (1/mask_ratio)).sum() / (loss_mask.sum() + 1e-8)
                    # print(f"[DEBUG] loss: {loss}")
                    # print(f"[DEBUG] hidden_states: {hidden_states}")
                    loss = (loss * (1/mask_ratio)).sum() / (loss_mask.sum() + 1e-8)
                else:
